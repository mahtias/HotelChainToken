...
contract HotelInvestmentToken is ERC20, Ownable, ReentrancyGuard {
    ...
    event InvestmentMade(address indexed investor, uint256 amount, uint256 tokens, uint256 timestamp);

    event DividendPaid(address indexed investor, uint256 amount, uint256 timestamp);

    event FundingGoalReached(uint256 totalFunding, uint256 timestamp);
    ...
    function invest() public payable nonReentrant {
        ...
        require(
            hotelInvestment.currentFunding.add(msg.value) <= hotelInvestment.fundingGoal, "Would exceed funding goal"
        );
    ...
    function getInvestorDetails(address investor)
        public
        view
        returns (uint256 investmentAmount, uint256 tokenBalance, uint256 sharePercentage, uint256 investmentDate)
    {
        return (
            investmentAmounts[investor], balanceOf(investor), getInvestorShare(investor), investmentTimestamps[investor]
        );
    }
